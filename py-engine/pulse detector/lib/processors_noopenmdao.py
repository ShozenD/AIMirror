...
class findFaceGetPulse(object):
    ...
    def run(self, cam):
        self.recorder.record("time", self.times)
        self.recorder.record("bpm", self.bpm)
        ...
        if self.find_faces:
            ...
            found_face = self.face_cascade.detectMultiScale(self.gray,
                                                            scaleFactor=1.3,
                                                            minNeighbors=4,
                                                            minSize=(
                                                                50, 50),
                                                            flags=cv2.CASCADE_SCALE_IMAGE)
            detected = list(found_face)
            if len(detected) > 0:
        ...
        cv2.putText(self.frame_out, "Press 'H' to toggle analysis plot",
